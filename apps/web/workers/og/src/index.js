// Cloudflare Worker that renders the share-link Open Graph cards (1200×630 PNG).
//
//   ?u=<username>          → profile card
//   ?list=<uuid>           → list card
...
// Set once from env on first request (env is constant for this Worker).
let TMDB_KEY = '';
let FONT_BASE = '';
...
const ACCENT = '#F06A88'; // colors.dark.accent (the primary pink)
...
// ── TMDB lookup (port of api/_tmdb.js loadTitle; key comes from env) ──────────
const TMDB = 'https://api.themoviedb.org/3';
const posterUrl = (path, size = 'w500') => (path ? `https://image.tmdb.org/t/p/${size}${path}` : null);
const normalizeType = (t) => (t === 'tv' || t === 'show' || t === 'series' ? 'tv' : 'movie');

async function loadTitle(mediaType, tmdbId) {
  const id = Number(tmdbId);
  if (!Number.isInteger(id) || id <= 0) return null;
  if (!TMDB_KEY) return null;
  const type = normalizeType(mediaType);
  const isV4 = TMDB_KEY.startsWith('eyJ'); // v4 read tokens are JWTs
  const url = `${TMDB}/${type}/${id}${isV4 ? '' : `?api_key=${encodeURIComponent(TMDB_KEY)}`}`;
  const init = isV4 ? { headers: { Authorization: `Bearer ${TMDB_KEY}` } } : undefined;
  let res;
...
function profileCard(profile, fonts) {
  const opts = { width: 1200, height: 630, fonts, headers: { 'cache-control': OG_CACHE } };
...
const handler = {
  async fetch(request, env) {
    ...
    if (listId) {
      ...
      return listCard(list, await loadFonts());
    }
    ...
    if (postId) {
      ...
      return postCard(post, await loadFonts());
    }

    // Title card — ?type=movie&id=123
    const id = searchParams.get('id');
    if (id) {
      let t = null;
      try { t = await loadTitle(searchParams.get('type'), id); } catch { /* generic card */ }
      return titleCard(t, await loadFonts());
    }
