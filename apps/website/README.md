...
Two notes on the command:

- `--compatibility-date` is pinned because `wrangler pages dev` otherwise defaults to *today's* date, which a slightly older `workerd` binary refuses to start on (`requires compatibility date "…", but the newest date supported by this server binary is "…"`). Pinning a past date is always safe; the runtime keeps supporting old dates.
- It runs from `apps/website` because Pages Functions are discovered relative to the working directory. From the repo root, wrangler picks up the *web app's* root `functions/` instead and this site's routes 404 — which looks identical to the static-server failure above.
