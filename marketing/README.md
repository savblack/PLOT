...
Sat 22:00 UTC (~8am Sun AEST)  marketing-weekly-batch.yml
           ...
           AI copy (Claude Code) — mkt:copy:pull -> write copy -> mkt:copy:save
                           -> status copy_ready  (subscription, no API key;
                            CLAUDE_CODE_OAUTH_TOKEN; see marketing/copy/AGENT.md)
...
Copy is generated by a scheduled coding agent — **Claude Code** (subscription via
`CLAUDE_CODE_OAUTH_TOKEN`), run inside the weekly batch. It's
...
6. **Copy worker**: the copy step lives in `.github/workflows/marketing-weekly-batch.yml`
   (Claude Code, headless, inside the weekly batch). It's inert until you add one
   secret — **`CLAUDE_CODE_OAUTH_TOKEN`** (subscription, *not* an API key; generate
   via `claude setup-token`). It reuses the existing `VITE_SUPABASE_URL`,
   `SUPABASE_SERVICE_ROLE_KEY`, `TMDB_API_KEY`, `OMDB_API_KEY` secrets. To run it
   with another agent (e.g. Codex), swap the agent step + token; the `AGENT.md`
   contract is unchanged. (`OMDB_API_KEY` is a free omdbapi.com key for ratings;
   without it, posts just omit ratings.)
