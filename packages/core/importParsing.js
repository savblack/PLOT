 ...
 * @property {string | null} [year] Four-digit release year when supplied.
 * @property {number | null} [rating] User rating on Plot's 1–10 scale.
 * @property {string | null} [note] Review or note supplied by the source.
 * @property {string | null} [externalId] Stable external title identifier.
 */

/** @typedef {'netflix' | 'prime' | 'disney' | 'max' | 'apple' | 'letterboxd' | 'imdb'} ImportPlatform */
...
export function parsePlatform(platformId, text) {
  switch (platformId) {
    ...
    case 'imdb':       return parseImdb(text);
    default:           return [];
  }
}
